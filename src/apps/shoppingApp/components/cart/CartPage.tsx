...
export default function CartPage() {
	...
	const isCartOpened: boolean = useSelector((state: RootState) => state?.cart?.isCartOpend);
	...
	const continnueShopping = () => {
		setCartOpened(false);
	};

	const removeProductFromCart = (product: Product) => {
		console.log("removeProductFromCart", product);
		dispatche(cartActions.removeFromCart(product));
	};

	return (
		<Dialog open={isCartOpened} onClose={() => setCartOpened(false)} className="relative z-10">
							...
							<div className="flex h-full flex-col overflow-y-scroll bg-white shadow-xl">
								<div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6">
									<div className="flex items-start justify-between">
										...
										<div className="ml-3 flex h-7 items-center">
											<button type="button" onClick={() => setCartOpened(false)} className="relative -m-2 p-2 text-gray-400 hover:text-gray-500">
													...
													<li key={product.id} className="flex py-6">
														...
														<div className="ml-4 flex flex-1 flex-col">
															...
															<div className="flex flex-1 items-end justify-between text-sm">
																...
																<div className="flex">
																	<button onClick={() => removeProductFromCart(product)} type="button" className="cursor-pointer font-medium text-indigo-600 hover:text-indigo-500">
																		Remove
																	</button>
																</div>
															</div>
								...
								<div className="border-t border-gray-200 px-4 py-6 sm:px-6">
									...
									<div className="mt-6 flex justify-center text-center text-sm text-gray-500">
										<p>
											or{" "}
											<button type="button" onClick={() => continnueShopping()} className="font-medium text-indigo-600 hover:text-indigo-500">
												Continue Shopping
